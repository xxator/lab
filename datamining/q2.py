...
def generateFrequentItemSet(CandidateList, noOfTransactions, minimumSupport, dataSet, fatherFrequentArray):
    ...
    for i in range(len(frequentItemsArray)):
        if i%2 == 0:
            if len(frequentItemsArray[i])==2:
                flag = 1

    if len(frequentItemsArray) == 2 or len(frequentItemsArray) == 0 or flag == 1:
...
for i in range(len(fatherFrequentArray)):
    if i%2 == 0:
        print(fatherFrequentArray[i]) 
